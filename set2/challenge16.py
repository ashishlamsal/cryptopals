...
def count_consecutive_zeros(buffer: bytes) -> int:
    """count the consecutive zeros starting from position 0 in the buffer.
    """
    count = 0
    for i in buffer:
        if i != 0:
...
def get_prefix_parameters(blocksize: int) -> tuple[int, bytes]:
    '''
    ...
    while True:
        padding += b'a'
        next_ciphertext = encrypt(padding)
        xored_ciphertext = repeating_key_xor(
            current_ciphertext, next_ciphertext)
